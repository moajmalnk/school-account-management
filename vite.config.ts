...
function appVersionPlugin(buildId: string): Plugin {
  const payload = JSON.stringify({ buildId, builtAt: new Date().toISOString() }, null, 0);
  return {
...
export default defineConfig(({ mode }) => {
  ...
  const apiProxyTarget = (env.VITE_API_BASE_URL?.trim() || "https://api.feezo.app").replace(
    /\/$/,
    "",
  );
    ...
    plugins: [
      ...
      VitePWA({
        ...
        manifest: {
          name: "Feezo",
          short_name: "Feezo",
          description: "Multi-tenant school management for students, staff, finance, and settings.",
          theme_color: "#0F766E",
          ...
          runtimeCaching: [
            {
              ...
              urlPattern: ({ sameOrigin, url }) => sameOrigin && url.pathname === "/version.json",
              handler: "NetworkOnly",
              options: {
