    ...
    >
      ...
      <div className="flex-1 bg-white overflow-hidden flex flex-col relative text-[var(--mkt-ink)]">
        ...
        >
          ...
          <nav
            className="flex gap-1 overflow-x-auto px-6 py-2 lg:hidden [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
          ...
          >
            {MARKETING.nav.map((item) => (
              <a
                key={item.id}
                href={`#${item.id}`}
                className="shrink-0 rounded-full px-3 py-1.5 text-[12px] font-medium text-[var(--mkt-muted)] hover:bg-[var(--mkt-soft)] hover:text-[var(--mkt-ink)] transition-all duration-200 hover:scale-105 active:scale-95"
              >
                {item.label}
              </a>
            ))}
        ...
        <footer
          className="text-[var(--mkt-ink)] relative overflow-hidden"
          style={{
            background: "linear-gradient(135deg, rgba(244,246,255,0.95) 0%, rgba(255,255,255,0.98) 50%, rgba(244,246,255,0.95) 100%)",
            borderTop: "1px solid rgba(143,202,74,0.2)",
          }}
        >
          {/* Decorative floating orbs */}
          <div className="absolute -top-16 -left-16 w-56 h-56 rounded-full bg-[var(--mkt-green)]/10 blur-3xl pointer-events-none" />
          <div className="absolute -bottom-16 -right-16 w-72 h-72 rounded-full bg-[var(--mkt-green)]/8 blur-3xl pointer-events-none" />
          <div className="mx-auto flex max-w-6xl flex-col gap-8 px-4 py-12 sm:px-6 md:flex-row md:items-start md:justify-between">
            ...
            <div className="flex flex-col gap-3 md:items-end pt-2">
              <a href="#" className="inline-flex h-[44px] items-center justify-start rounded-md bg-[var(--mkt-green)] px-3 text-white transition-colors hover:bg-[var(--mkt-green-deep)] w-[145px] gap-2 shadow-[0_4px_16px_rgba(143,202,74,0.4)]">
              ...
              <a href="#" className="inline-flex h-[44px] items-center justify-start rounded-md border border-[var(--mkt-line)] bg-white px-3 text-[var(--mkt-ink)] transition-colors hover:border-[var(--mkt-green)] hover:text-[var(--mkt-green)] w-[145px] gap-2 shadow-[0_4px_16px_rgba(143,202,74,0.08)]">
                ...
                <div className="flex flex-col items-start justify-center leading-none">
                  <span className="text-[9px] font-semibold tracking-wide mb-[3px] opacity-70">Download on the</span>
